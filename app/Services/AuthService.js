...
class AuthService {
  ...
  _registerPasswordRules() {
    owasp.config(this.config.passwordRules)
  }
  ...
  async updateEmail(user, newEmail) {
    ...
    }, this.updateEmailRules(user.primaryKeyValue), 'emailUpdate')
  ...
  async verify(payload, callback) {
    ...
    await this.runValidation(payload, this.config.loginRules, 'verify')
    const user = await this._getUserByUids(payload.uid)
  ...
  async runValidation(payload, rules, action) {
    const validation = await this.Validator.validateAll(payload, rules, this._getMessages(action))
  ...
  async login({
    ...
    await this.runValidation(user, this.config.loginRules, 'login')
  ...
  async register(data, loggedUser) {
    ...
    const newAccountState = activationRequired ? this.config.newAccountState : this.config.verifiedAccountState
    ...
    if (this.config.requiresStrongPassword) {
      ...
      const passwordValidationResult = await this.validatePassword(data[this.config.password])
